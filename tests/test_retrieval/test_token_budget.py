...
def test_chunk_tokens_clamped_to_at_least_one():
    """Derived chunk sizes must never drop to 0, even with severely constrained budgets."""
    for avail in (0, 1, 2, 3):
        b = TokenBudget(
            context_window=avail,
            reserve_output=0,
            reserve_system=0,
        )
        assert b.available == avail
        assert b.level1_chunk_tokens >= 1
        assert b.level2_chunk_tokens >= 1
        assert b.level2_refine_chunk_tokens >= 1
